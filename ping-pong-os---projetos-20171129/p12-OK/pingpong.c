...
int sem_destroy (semaphore_t *s)
{
    ...
    {
        check = s->fila;
        queue_remove((queue_t**)&s->fila, (queue_t*)check);
        check->estado = PRONTA;
        queue_append((queue_t**)&prontas, (queue_t*)check);
    }
    while(s->fila != NULL);
...
int barrier_join (barrier_t *b)
{
    ...
    {
        CurrentTask->estado = SUSPENSA;
        queue_append((queue_t**)&(b->tasks), (queue_t*)CurrentTask);
        flag_preempcao = 0;
        task_yield();
    }
    else
    {
        ...
        {
            check = b->tasks;
            queue_remove((queue_t**)&(b->tasks),(queue_t*)check);
            check->estado = PRONTA;
            queue_append((queue_t**)&prontas, (queue_t*)check);
            b->nTasks++;
        }
...
int mqueue_send (mqueue_t *queue, void *msg)
{
    ...
    bcopy(msg, queue->mensagens + (queue->qnt * queue->tam), queue->tam);
    queue->qnt++;
...
int mqueue_recv (mqueue_t *queue, void *msg)
{
    ...
    bcopy(queue->mensagens, msg, queue->tam);
    queue->qnt--;
